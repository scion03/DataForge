...
bool validKey(const std::string &to_check)
{
  std::set<std::string> keys = {"username", "email", "id"};
  return keys.find(to_check) != keys.end();
}
...
PrepareResult prepare_select(InputBuffer *input_buffer, Statement *statement)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    attr[i].key = strtok(NULL, "=");
    attr[i].val = strtok(NULL, " ");
    // std::cout.flush();
    if (attr[i].key == NULL)
      isValid = 0;
    else if (!isValid)
      return PREPARE_SYNTAX_ERROR;
    // key is valid but there is no value corresponding to it
    if (!isValid && attr[i].val != NULL)
      return PREPARE_SYNTAX_ERROR;
    if (attr[i].key != NULL && !validKey(attr[i].key))
      return PREPARE_SYNTAX_ERROR;
    if (attr[i].key != NULL && attr[i].val != NULL)
    {
      std::string s = attr[i].key, val = attr[i].val;
      mp[s] = val;
    }
  }
  ...
  if (id != -1)
  {
    ...
    statement->attributes_to_select["id"] = 1;
  }
  else
    statement->attributes_to_select["id"] = 0;
  if (username != "")
  {
    strcpy(statement->row_to_select.username, username.c_str());
    statement->attributes_to_select["username"] = 1;
  }
  else
    statement->attributes_to_select["username"] = 0;
  if (email != "")
  {
    strcpy(statement->row_to_select.email, email.c_str());
    statement->attributes_to_select["email"] = 1;
  }
  else
    statement->attributes_to_select["email"] = 0;
  // std::cout << statement->attributes_to_select["id"] << ' ' << statement->row_to_select.id << '\n';
  // std::cout << statement->attributes_to_select["username"] << ' ' << statement->row_to_select.username << '\n';
  // std::cout << statement->attributes_to_select["email"] << ' ' << statement->row_to_select.email << '\n';
...
bool validate_row(Row *row, Statement *statement)
{
  // print_row(row);
  // std::cout << statement->attributes_to_select["id"] << ' ' << statement->row_to_select.id << '\n';
  // std::cout << statement->attributes_to_select["username"] << ' ' << statement->row_to_select.username << '\n';
  ...
  if (statement->attributes_to_select["id"] && !(row->id == statement->row_to_select.id))
    return false;
  if (statement->attributes_to_select["username"] && (strcmp(row->username, statement->row_to_select.username)))
    return false;
  if (statement->attributes_to_select["email"] && (strcmp(row->email, statement->row_to_select.email)))
    return false;
  // print_row(row);
  return true;
}
